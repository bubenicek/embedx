...
osThreadId osThreadCreate (const osThreadDef_t *thread_def, void *argument)
{
  ...
  if (xTaskCreate((TaskFunction_t)thread_def->pthread,(const portCHAR *)thread_def->name,
              thread_def->stacksize / 4, argument, makeFreeRtosPriority(thread_def->tpriority),
              &handle) != pdPASS)  {
    return NULL;
  }
...
{
  timer_params_t *params = pvTimerGetTimerID(timer);
  if (params->callback_func != NULL)
    params->callback_func(params->argument);
}
...
osTimerId osTimerCreate (const osTimerDef_t *timer_def, os_timer_type type, void *argument)
{
  ...
  if ((params = osMemAlloc(sizeof(timer_params_t))) == NULL) {
    return NULL;
  }
  params->callback_func = timer_def->ptimer;
  params->argument = argument;

  timerId = xTimerCreate((const char *)"",
...
osStatus osTimerDelete (osTimerId timer_id)
{
  osStatus result = osOK;
   ...
   if (xPortInIsrContext()) {
    ...
    timer_params_t *params = pvTimerGetTimerID(timer_id);
    if (params != NULL) {
      osMemFree(params);
    }
...
osStatus osTimerStart (osTimerId timer_id, uint32_t millisec)
{
  ...
  if (xTimerIsTimerActive(timer_id) != pdFALSE)
  {
    if (xPortInIsrContext())
    {
      xTimerChangePeriodFromISR(timer_id, ticks, &taskWoken);
    ...
    {
      xTimerChangePeriod(timer_id, ticks, 0);
...
osMutexId osMutexCreateRecursive (const osMutexDef_t *mutex_def)
{
  return xSemaphoreCreateRecursiveMutex();
}

osStatus osMutexWaitRecursive (osMutexId mutex_id, uint32_t millisec)
{
  TickType_t ticks;
...
osStatus osMutexReleaseRecursive (osMutexId mutex_id)
{
  osStatus result = osOK;
...
osStatus osMutexDeleteRecursive (osMutexId mutex_id)
{
  vQueueDelete(mutex_id);
  return osOK;
}
