...
void SX126xSetTxParams( int8_t power, RadioRampTimes_t rampTime )
{
    ...
    else // sx1262
    {
        ...
        uint8_t value = SX126xReadRegister( REG_TXCLAMP_CONFIG );    // JBa
        SX126xWriteRegister( REG_TXCLAMP_CONFIG, value|0x1E );       // JBa
        value = SX126xReadRegister( REG_TXCLAMP_CONFIG );    // JBa
...
//        SX126xWriteRegister( REG_OCP, 0x7f ); // current max 180mA for the whole device
    }
    buf[0] = power;
...
int8_t SX126xReadOCR( void )
{
    uint8_t ocr;
    ocr = SX126xReadRegister( REG_OCP );
    return ocr;
}

int8_t SX126xReadXTA( void )
{
    uint8_t ocr;
    ocr = SX126xReadRegister( REG_XTA_TRIM );
    return ocr;
}

int8_t SX126xReadXTB( void )
{
    uint8_t ocr;
    ocr = SX126xReadRegister( REG_XTA_TRIM );
    return ocr;
