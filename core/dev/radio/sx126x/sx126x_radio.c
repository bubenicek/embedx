...
//#define TimerGetCurrentTime    //GetTimerMiliseconds
...
//struct timer TxTimeoutTimer;
//struct timer RxTimeoutTimer;
...
bool RadioIsChannelFree( RadioModems_t modem, uint32_t freq, int16_t rssiThresh, uint32_t maxCarrierSenseTime )
{
    ...
    //carrierSenseTime = hal_time_ms();

    // Perform carrier sense for maxCarrierSenseTime
    while( hal_time_ms() < (maxCarrierSenseTime + maxCarrierSenseTime))
...
void RadioSend( uint8_t *buffer, uint8_t size )
{
    ...
    //timer_set(&TxTimeoutTimer, TxTimeout);
...
void RadioRx( uint32_t timeout )
{
    ...
    if( timeout != 0 )
    {
        //timer_set(&RxTimeoutTimer, timeout);
...
void RadioSetTxContinuousWave( uint32_t freq, int8_t power, uint16_t time )
{
    ...
    //timer_set(&RxTimeoutTimer, time  * 1e3);
