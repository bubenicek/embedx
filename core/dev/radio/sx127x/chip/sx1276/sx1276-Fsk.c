...
//    100000,         // RxBw
    ...
    true,           // CrcOn
    false,           // AfcOn    
    9//255             // PayloadLength (set payload size to the maximum for variable mode, else set the exact payload length)
};
...
static int32_t RxPacketFeiValue;
static int32_t RxPacketAfcValue;
...
void SX1276FskInit( void )
{
    ...
    if( FskSettings.AfcOn == true )
    {
        ...
        SX1276->RegAfcFei =   RF_AFCFEI_AFCAUTOCLEAR_ON;
    }
    else
    {
...
//    return ( int32_t )( double )( ( ( int16_t )SX1276->RegFeiMsb << 8 ) | ( int16_t )SX1276->RegFeiLsb ) * ( double )FREQ_STEP;
    return ((int32_t) (((int16_t)(( int16_t )SX1276->RegFeiMsb << 8 ) | ( int16_t )SX1276->RegFeiLsb)*( double )FREQ_STEP ));
}

int32_t SX1276FskReadAfc( void )
{
    SX1276ReadBuffer( REG_AFCMSB, &SX1276->RegAfcMsb, 2 );                            // Reads the AFC value
    return ((int32_t) (((int16_t)(( int16_t )SX1276->RegAfcMsb << 8 ) | ( int16_t )SX1276->RegAfcLsb)*( double )FREQ_STEP ));
...
//        PacketTimeout = ( uint16_t )( round( ( 8.0 * ( ( double )SX1276FskGetPacketPayloadSize( ) ) / ( double )FskSettings.Bitrate ) * 1000.0 ) + 1.0 );
        PacketTimeout = ( uint16_t )( ( ( 8L * ( SX1276FskGetPacketPayloadSize( ) ) / FskSettings.Bitrate ) * 1000.0 ) + 2.0 );
    ...
    case RF_STATE_RX_SYNC:
        ...
        {
            RxPacketFeiValue = SX1276FskReadFei();
