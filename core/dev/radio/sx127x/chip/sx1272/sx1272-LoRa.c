...
//#include "IO_port_DVB1v1.h"
//#include "dvb_cfg_struct.h"

#include "platform/platform.h"

TRACE_TAG(sx1272lora);
...
//tLoRaSettings *LoRaSettings = DVBCfg->Sys;
...
//    true,             // CrcOn [0: OFF, 1: ON]
   ...
   #if (SCUBA_DIVER_TEST>0)
    18,               // PayloadLength (used for implicit header mode)
   #else
    13,               // PayloadLength (used for implicit header mode)
   #endif
...
void SX1272LoRaInit( void )
{
    ...
    // TODO: RBU fix
    //memcpy(&LoRaSettings, &DVBCfg->Sys.LoRaSettings, sizeof(tLoRaSettings));
    
    SX1272LoRaSetDefaults( );
    
    SX1272ReadBuffer( REG_LR_OPMODE, SX1272Regs + 1, 0x70 - 1 );

#if (( PLATFORM == SX12xxEiger ) || ( PLATFORM == SX12xxDVB ))
    ...
    SX1272LoRaSetPreambleLength(LoRaSettings.PreambleLen);
...
/*
    ...
    {
     SX1272LoRaSetPayloadLength( 255);
    }
  */  
    if ( ( ( LoRaSettings.SignalBw == 0 ) && ( ( LoRaSettings.SpreadingFactor == 11 ) || ( LoRaSettings.SpreadingFactor == 12 ) ) ) ||
    ( ( LoRaSettings.SignalBw == 1 ) && ( LoRaSettings.SpreadingFactor == 12 ) ) )
    {
     SX1272LoRaSetLowDatarateOptimize( true );
    }
    else
    {
     SX1272LoRaSetLowDatarateOptimize( false );
    }
//    SX1272LoRaSetLowDatarateOptimize( true );
...
int16_t SX1272LoRaReadRssi( void )
{
    int16_t rssi = 0;

   SX1272Read( REG_LR_RSSIVALUE, &SX1272LR->RegRssiValue );
   rssi = SX1272LR->RegRssiValue;
   rssi += RSSI_OFFSET;
   return rssi;
}
...
//            SX1272LR->RegHopPeriod = 255;
    ...
    case RFLR_STATE_TX_INIT:
        ...
        if( LoRaSettings.ImplicitHeaderOn == true )
         SX1272LR->RegPayloadLength = LoRaSettings.PayloadLength;
        else 
