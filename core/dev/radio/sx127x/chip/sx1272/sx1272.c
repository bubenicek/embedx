...
int SX1272Init( void )
{
    ...
    SX1272Read(REG_LR_VERSION, &ver);
    TRACE("%s version: 0x%X", __FUNCTION__, ver);

    if (ver != 0x22)
    {
        TRACE_ERROR("Can't find SX1272 chip");
        return -1;
    }
...
void SX1227DumpRegisters(const char *title)
{
    uint8_t reg;

    TRACE("SX1227 registers - '%s'", title);
    TRACE("-------------------------------------------------");
    for (int addr = 0; addr <= 0x46; )
    {
        for (int i = 0; i < 8; i++, addr++)
        {
            SX1272Read(addr, &reg);
            TRACE_PRINTF("   0x%2.2X = 0x%2.2X", addr, reg);
        }
        TRACE_PRINTF("\n");
    }
}
