...
void IT8951_Refresh(int mode)
{
	...
	// Display function 2
	IT8951DisplayArea(0, 0, gstI80DevInfo.usPanelW, gstI80DevInfo.usPanelH, mode);
}

void IT8951_writeBitmap(uint8_t *bmp, int x, int y, int width, int height, int mode)
{
	IT8951LdImgInfo stLdImgInfo;
	IT8951AreaImgInfo stAreaImgInfo;
	...
	// Setting Load image information
	stLdImgInfo.ulStartFBAddr    = (uint32_t)bmp;
	stLdImgInfo.usEndianType     = IT8951_LDIMG_L_ENDIAN;
	stLdImgInfo.usPixelFormat    = IT8951_8BPP;
	stLdImgInfo.usRotate         = IT8951_ROTATE_0;
	stLdImgInfo.ulImgBufBaseAddr = gulImgBufAddr;
	...
	// Display function
	IT8951DisplayArea(x, y, width, height, mode);
}
