...
bool led_strip_init(struct led_strip_t *led_strip)
{
    ...
    {
		  ESP_LOGE(TAG, "LED strip init check failed");
        return false;
    }

    if(led_strip->led_strip_buf_1 == led_strip->led_strip_buf_2) {
        ESP_LOGE(TAG, "LED strip init - bad buffers");
        return false;
    }
    ...
    if (!init_rmt) {
        ESP_LOGE(TAG, "LED strip init_rmt failed");
        return false;
    }
    ...
    if (!task_created) {
        ESP_LOGE(TAG, "LED strip init - create refresh task failed");
        return false;
    }
