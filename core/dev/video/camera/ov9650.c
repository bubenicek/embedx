...
/* TODO
    ...
    ret |= SCCB_Write(sensor->slv_addr, REG_COM8, (reg & (~REG_COM8_AGC)) | ((enable != 0) ? REG_COM8_AGC : 0));
    ...
    if ((enable == 0) && (!isnanf(gain_db)) && (!isinf(gain_db))) {
        ...
        ret |= SCCB_Write(sensor->slv_addr, REG_GAIN, ((gain_hi & 0x0F) << 4) | (gain_lo << 0));
        ret |= cambus_readb(sensor->slv_addr, REG_VREF, &reg);
        ret |= SCCB_Write(sensor->slv_addr, REG_VREF, ((gain_hi & 0x30) << 2) | (reg & 0x3F));
        ...
        ret |= SCCB_Write(sensor->slv_addr, REG_COM9, (reg & 0x8F) | ((fast_ceilf(fast_log2(gain_ceiling)) - 1) << 4));
    }
...
static int get_gain_db(sensor_t *sensor, float *gain_db)
{
    ...
    reg = SCCB_Read(sensor->slv_addr, REG_COM8);

    // DISABLED
    // if (reg & REG_COM8_AGC) {
    //     ret |= SCCB_Write(sensor->slv_addr, REG_COM8, reg & (~REG_COM8_AGC));
    // }
    // DISABLED

    gain_lo = SCCB_Read(sensor->slv_addr, REG_GAIN);
    gain_hi = SCCB_Read(sensor->slv_addr, REG_VREF);
...
/* TODO
    reg = SCCB_Read(sensor->slv_addr, REG_COM8);
    ret |= SCCB_Write(sensor->slv_addr, REG_COM8, (reg & (~REG_COM8_AEC)) | ((enable != 0) ? REG_COM8_AEC : 0));

    if ((enable == 0) && (exposure_us >= 0)) {
        reg = SCCB_Read(sensor->slv_addr, REG_COM7);
        ...
        reg = SCCB_Read(sensor->slv_addr, REG_CLKRC);
        ...
        ret |= SCCB_Write(sensor->slv_addr, REG_COM1, (reg & 0xFC) | ((exposure >> 0) & 0x3));

        ret |= cambus_readb(sensor->slv_addr, REG_AECH, &reg);
        ret |= SCCB_Write(sensor->slv_addr, REG_AECH, (reg & 0x00) | ((exposure >> 2) & 0xFF));

        ret |= cambus_readb(sensor->slv_addr, REG_AECHM, &reg);
        ret |= SCCB_Write(sensor->slv_addr, REG_AECHM, (reg & 0xC0) | ((exposure >> 10) & 0x3F));
    }
*/
    return ret;
}
...
/* TODO
    ...
    // DISABLED
    // if (reg & REG_COM8_AEC) {
    //     ret |= SCCB_Write(sensor->slv_addr, REG_COM8, reg & (~REG_COM8_AEC));
    // }
    // DISABLED
...
/* TODO
    uint8_t reg;
    int ret = cambus_readb(sensor->slv_addr, REG_COM8, &reg);
    ret |= SCCB_Write(sensor->slv_addr, REG_COM8, (reg & (~REG_COM8_AWB)) | ((enable != 0) ? REG_COM8_AWB : 0));
    ...
    if ((enable == 0) && (!isnanf(r_gain_db)) && (!isnanf(g_gain_db)) && (!isnanf(b_gain_db))
        ...
        ret |= SCCB_Write(sensor->slv_addr, REG_BLUE, b_gain);
        ret |= SCCB_Write(sensor->slv_addr, REG_RED, r_gain);
    }
...
static int get_rgb_gain_db(sensor_t *sensor, float *r_gain_db, float *g_gain_db, float *b_gain_db)
{
    ...
    // DISABLED
    // if (reg & REG_COM8_AWB) {
    //     ret |= SCCB_Write(sensor->slv_addr, REG_COM8, reg & (~REG_COM8_AWB));
    // }
    // DISABLED

    blue = SCCB_Read(sensor->slv_addr, REG_BLUE);
    red = SCCB_Read(sensor->slv_addr, REG_RED);
    ...
    *r_gain_db = 20.0 * (log(red / 128.0) / log(10.0));
    *b_gain_db = 20.0 * (log(blue / 128.0) / log(10.0));
...
static int set_hmirror(sensor_t *sensor, int enable)
{
   ...
   ret |= SCCB_Write(sensor->slv_addr, REG_MVFP, enable ? (val | REG_MVFP_HMIRROR) : (val & (~REG_MVFP_HMIRROR)));
...
static int set_vflip(sensor_t *sensor, int enable)
{
   ...
   val = SCCB_Read(sensor->slv_addr, REG_MVFP);
   ret |= SCCB_Write(sensor->slv_addr, REG_MVFP, enable ? (val | REG_MVFP_VFLIP) : (val & (~REG_MVFP_VFLIP)));
