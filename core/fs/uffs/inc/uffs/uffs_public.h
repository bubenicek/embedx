...
#define TRACE_TAG "uffs"

#define uffs_debug(_fmt, ...)  TRACE(_fmt, ##__VA_ARGS__)

#else
    //
    // Debug disabled
    //
#define uffs_debug(_fmt, ...)

#endif   // CONFIG_ENABLE_UFFS_DEBUG_MSG


#define uffs_DebugMessage(_level, _prefix, _suffix, _errFmt, ...)   uffs_debug(_prefix _errFmt, ##__VA_ARGS__)
#define uffs_AssertCall(_file, _line, _msg, ...) uffs_debug(_msg, ##__VA_ARGS__)
#define uffs_Perror(level, fmt, ... ) uffs_debug(fmt, ##__VA_ARGS__)
#define uffs_PerrorRaw(level, fmt, ... ) uffs_debug(fmt, ##__VA_ARGS__)

#define uffs_Assert(expr, msg, ...) \
({ \
    int __res; \
    if (!(expr)) \
		 uffs_debug("ASSERT FAILED: " msg, ##__VA_ARGS__); \
	__res = expr ? U_TRUE : U_FALSE; \
	__res; \
})
