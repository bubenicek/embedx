...
int BlueNRG_Stack_Initialization(void)
{
    uint8_t index;
    ...
    if ((hci_mutex = osMutexCreate(NULL)) == NULL)
    {
        TRACE_ERROR("Create HCI mutex");
        return -1;
    }
...
static int HCI_read_packet(uint8_t *buf, int bufsize)
{
    int res;
    int datalen;
    ...
    // Test event packet type
    if (buf[0] != HCI_EVENT_PKT)
    {
        TRACE_ERROR("Not valid HCI event packet type 0x%X != 0x%X", buf[0], HCI_EVENT_PKT);
        return -1;
    }
    
    // Test data size
    datalen = buf[2];
    if (datalen > HCI_READ_PACKET_SIZE - 3)
    {
        TRACE_ERROR("HCI packet length: %d out of buffer size: %d", datalen, HCI_READ_PACKET_SIZE);
        return -1;
    }

    // Read packet data
    res = HAL_Read_Serial(&buf[3], datalen, 1000);
    if (res != datalen)
    {
        TRACE_ERROR("Recv HCI data failed");
        return -1;
    }
...
void HCI_Isr(void)
{
    ...
    if (list_is_empty(&hciReadPktPool) == FALSE)
    {
        ...
        // Read packet 
        data_len = HCI_read_packet(hciReadPacket->dataBuff, HCI_READ_PACKET_SIZE);
        if (data_len > 0)
        {
#ifdef HCI_LOG_ON            
            TRACE_PRINTFF("HCI RX (%02d) -> ", data_len);
            for (int i = 0; i < data_len; i++) {
                TRACE_PRINTF("%02X ", hciReadPacket->dataBuff[i]);
            }
            TRACE_PRINTF("\n");
#endif
            ...
            {
                TRACE_ERROR("Verify hci packet failed, length: %d", data_len);
        ...
        {
            if (data_len < 0) {
                TRACE_ERROR("Receive hci packet failed");
            }
