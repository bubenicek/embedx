...
{
   uint32_t uuid;
   ip_addr_t ipaddr = {.addr = cfg->ipaddr};
   ip_addr_t netmask = {.addr = cfg->netmask};
   ip_addr_t gw = {.addr = cfg->gw};
   ...
   if (cfg->dhcp_enabled)
   {
      ...
      TRACE("Net if init, MAC addr: %2.2X:%2.2X:%2.2X:%2.2X:%2.2X:%2.2X  dhcp started",
