...
TRACE_TAG(dac_signal);

TM_DAC_SIGNAL_Result_t TM_DAC_SIGNAL_SetCustomSignal(TM_DAC_SIGNAL_Channel_t DACx, uint16_t* Signal_Data, uint16_t Signal_Length, double frequency, double amplitude_offset);
...
uint16_t DAC_SIGNAL_Buffer[DAC_SIGNAL_BUFFER_SIZE] = {0, 1, 2, 3, 4};
...
TM_DAC_SIGNAL_Result_t TM_DAC_SIGNAL_DeInit(TM_DAC_SIGNAL_Channel_t DACx, TIM_TypeDef* TIMx)
{
	switch (DACx) {
		case TM_DAC1:
...
TM_DAC_SIGNAL_Result_t TM_DAC_SIGNAL_SetSignal(TM_DAC_SIGNAL_Channel_t DACx, TM_DAC_SIGNAL_Signal_t signal_type, double frequency, double amplitude_offset)
{
	TM_DAC_SIGNAL_Result_t result;

	switch (signal_type) {
		case TM_DAC_SIGNAL_Signal_Sinus:
			result = TM_DAC_SIGNAL_SetCustomSignal(DACx, DAC_SIGNAL_Sinus, DAC_SIGNAL_SINUS_LENGTH, frequency, amplitude_offset);
			break;
		case TM_DAC_SIGNAL_Signal_Sawtooth:
			result = TM_DAC_SIGNAL_SetCustomSignal(DACx, DAC_SIGNAL_Sawtooth, DAC_SIGNAL_SAWTOOTH_LENGTH, frequency, amplitude_offset);
			break;
		case TM_DAC_SIGNAL_Signal_Triangle:
			result = TM_DAC_SIGNAL_SetCustomSignal(DACx, DAC_SIGNAL_Triangle, DAC_SIGNAL_TRIANGLE_LENGTH, frequency, amplitude_offset);
			break;
		case TM_DAC_SIGNAL_Signal_Square:
			result = TM_DAC_SIGNAL_SetCustomSignal(DACx, DAC_SIGNAL_Square, DAC_SIGNAL_SQUARE_LENGTH, frequency, amplitude_offset);
			break;
		default:
...
TM_DAC_SIGNAL_Result_t TM_DAC_SIGNAL_SetCustomSignal(TM_DAC_SIGNAL_Channel_t DACx, uint16_t* Signal_Data, uint16_t Signal_Length, double frequency, double amplitude_offset)
{
	...
	memset(&TIM_TimeBaseStruct, 0, sizeof(TIM_TimeBaseStruct));
	memset(&Timer_Data, 0, sizeof(Timer_Data));

	ASSERT(Signal_Length <= DAC_SIGNAL_BUFFER_SIZE);

	// Prepare output signal buffer
	for (ix = 0; ix < Signal_Length; ix++)
	{
        DAC_SIGNAL_Buffer[ix] = Signal_Data[ix] * amplitude_offset;
	}
	...
	/* Set DMA options */
	DMA_InitStruct.DMA_Memory0BaseAddr = (uint32_t)&DAC_SIGNAL_Buffer[0];
